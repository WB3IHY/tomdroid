...
public abstract class WebConnection {
	...
	protected String execute(HttpUriRequest request) throws UnknownHostException {
		...
		try {
			...
			// getNewHttpClient() builds a fresh connection pool for every single request;
			// shut it down once the response body has been fully read above, otherwise its
			// pooled connections and background thread leak until the GC finalizes them.
