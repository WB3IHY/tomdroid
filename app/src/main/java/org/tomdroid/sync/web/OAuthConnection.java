...
public class OAuthConnection extends WebConnection {
	...
	// httpclient's lifetime is owned by the caller, which must shut its connection manager
	// down (in a finally block) once done with the returned provider
	private OAuthProvider getProvider(DefaultHttpClient httpclient) {
	...
	public Uri getAuthorizationUrl(String server) throws UnknownHostException {
		...
		OAuthProvider provider = getProvider(httpclient);
		...
		TLog.i(TAG, "Authorization URL retrieved");
	...
	public boolean getAccess(String verifier) throws UnknownHostException {
		...
		if (!requestToken.equals("") && !requestTokenSecret.equals("")) {
			...
			TLog.d(TAG, "Added request token");
		}
		else
			return false;
