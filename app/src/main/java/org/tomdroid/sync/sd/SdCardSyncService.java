...
public class SdCardSyncService extends SyncService {
	...
	// guards against a malicious/corrupted guid (e.g. from a compromised sync server)
	// escaping the notes directory when used as a filename
	private static boolean isSafeGuid(String guid) {
		return guid != null && guid.matches("[A-Za-z0-9-]+");
	}

	// actually pushes a note to sdcard, with optional subdirectory (e.g. backup)
	private static int doPushNote(Note note) {

		if (!isSafeGuid(note.getGuid())) {
			TLog.e(TAG, "Refusing to push note with unsafe guid: {0}", note.getGuid());
	...
	private void deleteNote(String guid){
		if (!isSafeGuid(guid)) {
			TLog.e(TAG, "Refusing to delete note with unsafe guid: {0}", guid);
	...
	protected void pullNote(String guid) {
		...
		if (!isSafeGuid(guid)) {
			TLog.e(TAG, "Refusing to pull note with unsafe guid: {0}", guid);
