...
public class NoteManager {
	...
	public static Note getNote(Activity activity, Uri uri) {
		...
		Cursor cursor = activity.getContentResolver().query(uri, FULL_PROJECTION, null, null, null);
	...
	public static ListAdapter getListAdapter(Activity activity, String querys, int selectedIndex, String notebookFilter) {
		...
		if(notebookFilter != null)
			optionalQueries += NotebookManager.UNFILED.equals(notebookFilter) ? 1 : 4;
		...
		if (notebookFilter != null) {
			if (NotebookManager.UNFILED.equals(notebookFilter)) {
				where += (where.length() > 0? " AND ":"")+"(" + Note.TAGS + " NOT LIKE ?)";
				qargs[count++] = "%" + NotebookManager.TAG_PREFIX + "%";
			} else {
				// match the notebook tag anchored to comma boundaries (a plain "%tag%" LIKE
				// would also match e.g. notebook "Work" against a note tagged "Work Trip"),
				// and escape %/_ in the notebook name so it can't act as a SQL wildcard
				String tag = NotebookManager.tagFor(notebookFilter);
				String escapedTag = escapeLikeArg(tag);
				where += (where.length() > 0? " AND ":"")
					+ "(" + Note.TAGS + "=?"
					+ " OR " + Note.TAGS + " LIKE ? ESCAPE '\\'"
					+ " OR " + Note.TAGS + " LIKE ? ESCAPE '\\'"
					+ " OR " + Note.TAGS + " LIKE ? ESCAPE '\\')";
				qargs[count++] = tag;
				qargs[count++] = escapedTag + ",%";
				qargs[count++] = "%," + escapedTag;
				qargs[count++] = "%," + escapedTag + ",%";
			}
		}
	...
	// escapes % and _ (SQL LIKE wildcards) and the escape character itself, so a value used
	// inside a LIKE pattern is matched literally rather than as a wildcard
	private static String escapeLikeArg(String value) {
		return value.replace("\\", "\\\\").replace("%", "\\%").replace("_", "\\_");
	}

	public static ListAdapter getListAdapter(Activity activity, String querys, int selectedIndex) {
		return getListAdapter(activity, querys, selectedIndex, null);
	}
	public static ListAdapter getListAdapter(Activity activity, int selectedIndex) {

		return getListAdapter(activity, null, selectedIndex, null);
	}
	public static ListAdapter getListAdapter(Activity activity, String querys) {

		return getListAdapter(activity, querys, -1, null);
	}
	public static ListAdapter getListAdapter(Activity activity) {

		return getListAdapter(activity, null, -1, null);
	}
