...
/*			case R.id.edit_note_xml:
	...
	private void showNotebookPicker() {
		...
		String current = NotebookManager.getNotebook(note.getTags());

		final String[] items = new String[notebooks.size() + 2];
		items[0] = getString(R.string.noNotebook);
		for (int i = 0; i < notebooks.size(); i++)
			items[i + 1] = notebooks.get(i);
		items[items.length - 1] = getString(R.string.newNotebook);

		int checkedItem = 0;
		if (current != null) {
			int idx = notebooks.indexOf(current);
			if (idx >= 0)
				checkedItem = idx + 1;
		}

		new AlertDialog.Builder(this)
			.setTitle(R.string.menuNotebook)
			.setSingleChoiceItems(items, checkedItem, new DialogInterface.OnClickListener() {
				public void onClick(DialogInterface dialog, int which) {
					...
					if (which == 0) {
						...
						assignNotebook(notebooks.get(which - 1));
					}
				}
			})
		...
		new AlertDialog.Builder(this)
			...
			.setPositiveButton(R.string.btnOk, new DialogInterface.OnClickListener() {
				public void onClick(DialogInterface dialog, int which) {
					...
					if (name.length() == 0)
						return;
					NotebookManager.createNotebook(EditNote.this, name);
					assignNotebook(name);
				}
			})
			.setNegativeButton(R.string.btnCancel, null)
	...
	private void assignNotebook(String notebookName) {
		// make sure any text the user has typed but not yet saved is captured in `note`
		// before persisting, so this doesn't overwrite it with stale content
		updateNoteContent(xmlOn);
		NotebookManager.setNotebook(note, notebookName);
		note.setLastChangeDate();
		NoteManager.putNote(this, note);
		String message = notebookName == null
			? getString(R.string.messageRemovedFromNotebook)
			: String.format(getString(R.string.messageAddedToNotebook), notebookName);
