...
public class Tomdroid extends ActionBarListActivity {
	...
	public static final String	PROJECT_HOMEPAGE	= "https://github.com/WB3IHY/tomdroid";
	...
	// null means "All Notes"; may also be NotebookManager.UNFILED
	private String currentNotebookFilter = null;
	...
	public boolean onCreateOptionsMenu(Menu menu) {
		...
		// Hook the search item's action view up to a real SearchView. Its suggestions
		// dropdown is a normal in-app popup list (unlike the old floating system Search
		// Dialog triggered by startSearch(), whose suggestion list could not be scrolled
		// on many modern devices), so previous search entries stay reachable.
	...
	private void updateNotesList(String aquery, int aposition) {
		...
		adapter = NoteManager.getListAdapter(this, aquery, rightPane != null ? aposition : -1, currentNotebookFilter);
		setListAdapter(adapter);

	}

	private void showNotebookPicker() {
		final List<String> notebooks = NotebookManager.getAllNotebooks(this);
		final String[] items = new String[notebooks.size() + 3];
		items[0] = getString(R.string.allNotes);
		items[1] = getString(R.string.unfiledNotes);
		for (int i = 0; i < notebooks.size(); i++)
			items[i + 2] = notebooks.get(i);
		items[items.length - 1] = getString(R.string.newNotebook);

		new AlertDialog.Builder(this)
			.setTitle(R.string.menuNotebooks)
			.setItems(items, new DialogInterface.OnClickListener() {
				public void onClick(DialogInterface dialog, int which) {
					if (which == 0) {
						currentNotebookFilter = null;
					} else if (which == 1) {
						currentNotebookFilter = NotebookManager.UNFILED;
					} else if (which == items.length - 1) {
						promptNewNotebook();
						return;
					} else {
						currentNotebookFilter = notebooks.get(which - 2);
					}
					afterNotebookFilterChanged();
				}
			})
			.show();
	}

	private void promptNewNotebook() {
		final EditText input = new EditText(this);
		new AlertDialog.Builder(this)
			.setTitle(R.string.newNotebook)
			.setView(input)
			.setPositiveButton(R.string.btnOk, new DialogInterface.OnClickListener() {
				public void onClick(DialogInterface dialog, int which) {
					...
					if (name.length() == 0)
						return;
					NotebookManager.createNotebook(Tomdroid.this, name);
	...
	private void afterNotebookFilterChanged() {
		updateNotesList(query, 0);
		showNoteInPane(0);
		updateEmptyList(query);
		updateNotebookTitle();
	...
	private void updateNotebookTitle() {
		if (Intent.ACTION_SEARCH.equals(intent.getAction()))
			return;
		if (currentNotebookFilter == null)
			setTitle(R.string.app_name);
		else if (NotebookManager.UNFILED.equals(currentNotebookFilter))
			setTitle(getString(R.string.app_name) + " - " + getString(R.string.unfiledNotes));
		else
			setTitle(getString(R.string.app_name) + " - " + currentNotebookFilter);
	}
	...
	private void updateTextAttributes() {
		...
		title.setTextColor(getColor(R.color.note_title));
		title.setPaintFlags(title.getPaintFlags() | Paint.UNDERLINE_TEXT_FLAG);
		title.setBackgroundColor(getColor(R.color.note_background));
	...
	public void newNote() {
		...
		if (currentNotebookFilter != null && !NotebookManager.UNFILED.equals(currentNotebookFilter))
			NotebookManager.setNotebook(note, currentNotebookFilter);
