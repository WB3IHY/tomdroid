...
public class NoteListCursorAdapter extends SimpleCursorAdapter {
    ...
    public NoteListCursorAdapter (Context context, int layout, Cursor c, String[] from, int[] to, int selectedIndex) {
        ...
        // resolved once here rather than on every getView() call (once per visible row per
        // scroll frame); a day/night switch recreates the activity/adapter anyway
        noteTextColor = context.getColor(R.color.note_text);
    }
    ...
    // This is the sole place row text color is set (selected vs. unselected state) — the
    // layout XML intentionally leaves note_title/note_date's textColor unset.
    @Override
	public View getView(int position, View convertView, ViewGroup parent) {
    	...
    	if(this.selectedIndex == position) {
            ...
            TextView note_title = (TextView) v.findViewById(R.id.note_title);
            if (note_title != null) {
            	note_title.setTextColor(noteTextColor);
            }
            TextView note_modified = (TextView) v.findViewById(R.id.note_date);
            if (note_modified != null) {
            	note_modified.setTextColor(noteTextColor);
            }
